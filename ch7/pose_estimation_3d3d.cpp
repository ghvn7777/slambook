...
class EdgeProjectXYZRGBDPoseOnly : public g2o::BaseUnaryEdge<3, Eigen::Vector3d, g2o::VertexSE3Expmap> {
  public:
    ...
    virtual void linearizeOplus() {
        ...
        Eigen::Vector3d xyz_trans = T.map(_point); // 从 2 变换到 1
        double x = xyz_trans[0]; // 2 变换到 1 的相机坐标系
...
int main(int argc, char** argv) {
    ...
    for (DMatch m : matches) {
        ...
        pts1.push_back(Point3f(p1.x*dd1, p1.y*dd1, dd1)); //相机 1 的相机坐标系
        pts2.push_back(Point3f(p2.x*dd2, p2.y*dd2, dd2)); //相机 2 的相机坐标系
    }
    ...
    cout << "R = " << R << endl;
    cout << "t = " << t << endl;  // t 是旋转前的平移
    cout << "R_inv = " << R.t() << endl;
    cout << "t_inv = " << -R.t() * t << endl; // t 的逆运算是 旋转回来然后平移取负
...
void pose_estimation_3d3d(const vector<Point3f>& pts1,
    ...
    p1 = Point3f(Vec3f(p1) / N); //质心
    p2 = Point3f(Vec3f(p2) / N);

    vector<Point3f> q1(N), q2(N); // remove the center
    for (int i = 0; i < N; i++) {
        q1[i] = pts1[i] - p1;     // q_i = p_i - p
        q2[i] = pts2[i] - p2;
    }
    ...
    Eigen::Matrix3d R_ = U * V.transpose(); // R = U * V^T
    Eigen::Vector3d t_ = Eigen::Vector3d(p1.x, p1.y, p1.z) - R_ * Eigen::Vector3d(p2.x, p2.y, p2.z); // t = p - Rp'
