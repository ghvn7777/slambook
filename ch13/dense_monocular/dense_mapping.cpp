...
int main (int argc, char** argv) {
    ...
    for (int index=1; index < color_image_files.size(); index++) {
        ...
        Mat curr = imread(color_image_files[index], 0); // 读取灰度图
...
bool epipolarSearch(
    ...
    Vector3d f_ref = px2cam(pt_ref); // 当前帧该点的相机坐标系坐标
    f_ref.normalize();
    Vector3d P_ref = f_ref * depth_mu;	// 参考帧的 P 向量
    
    Vector2d px_mean_curr = cam2px(T_C_R * P_ref); // 按深度均值投影的像素
    double d_min = depth_mu - 3 * depth_cov; // 极线区间
    ...
    epipolar_direction.normalize();                     // 相当于变成这个方向的单位向量
    double half_length = 0.5 * epipolar_line.norm();	// 极线线段的半长度, norm 是求向量的模长
...
)
{
    ...
    // 方程
    // d_ref * f_ref = d_cur * ( R_RC * f_cur ) + t_RC  // 参见 7.5 三角测量
